...
| `1` | `translate` or `import` finished, but at least one locale failed or came out partial (a partial locale is one whose file was written with some keys still missing); for `check` and `diff`, at least one locale is out of sync; for `doctor`, at least one check failed; for `types --check`, the committed declaration is out of date. |
