...
`@verbatra/studio` (public, local dashboard). `apps/docs` (private) is the Fumadocs
documentation site. `@verbatra/cli` and `@verbatra/sdk` version together (Changesets
fixed group); `@verbatra/studio` versions independently.
...
- zod validates at boundaries only (config, CLI args, provider responses), not in
  hot paths.
- API keys come only from environment variables, read through
  `packages/ai-providers/src/env.ts`. Never from config files, CLI args, or function
  arguments. Never log or commit a key.
- Any publishable `src` change (including `@verbatra/studio`) ships a changeset with
  the correct bump level.
