...
- `GLOSSARY_TERM_MISSED`: a configured glossary term occurs in the source but its mapped target
  term is absent from the translation. Matching is case-insensitive and whole-word on both sides,
  so the term `AI` is not counted as present inside `Airport`. Scripts written without word
  separators (Han, kana, Thai and similar) have no boundary to anchor to, so a term whose edge
  falls in one of those is matched by plain containment instead.
