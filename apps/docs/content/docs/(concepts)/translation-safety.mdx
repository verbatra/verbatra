...
- Accepted: the same tags in a different order, because translating a sentence moves its markup
  ...
  and `<br/>` are one tag.
- Counted by presence rather than by occurrence for a value the format reports as one plural
  message, so a language needing four plural arms where English declares two is not refused for
  repeating the source's own markup.
...
mentions `<Enter>` is never held to a structure it never had. And it stands down per tag: a tag the
