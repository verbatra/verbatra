...
- Refused: a dropped tag, an invented tag, a renamed tag, a changed set of attribute names, and
  markup that comes back unbalanced or mis-nested, such as `<b>a<i>b</b></i>`.
- Accepted: the same tags in a different order, because translating a sentence moves its markup
  around, and a changed attribute value, because a translated `title` or a localized `href` is a
  correct translation rather than a broken one.
...
mentions `<Enter>` is never held to a structure it never had. And it never applies to a value whose
own format already reports its markup as placeholders: an XLIFF inline element and a next-intl or
ARB rich-text tag are covered by the placeholder check instead, and are never reported twice.
