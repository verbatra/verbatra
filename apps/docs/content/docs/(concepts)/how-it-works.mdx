...
Before anything is sent, every candidate is looked up in the local translation-memory cache
(`verbatra.cache.json`). A stored translation for the same source text, under the same provider,
model, tone and glossary fingerprint, is reused as-is and never reaches the provider; only the
misses go on. See [The cache](/docs/the-cache) for how entries are keyed and when they stop
matching.
...
DeepL and Google Cloud Translation are machine-translation APIs without this response layer, but
their results go through the same integrity checks below.
