...
The figure is an upper bound for the same two reasons the dry-run count is, and for a third: tokens are derived from character counts, not from the provider's own tokenizer. The `estimate excludes` line names all three on every run.
