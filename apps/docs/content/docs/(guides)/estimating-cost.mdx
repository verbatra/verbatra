...
The estimate covers `translate`. Retranslating a single entry from [Studio](/docs/review-in-studio) or from an agent tool calls the provider on its own path, and no estimate here sees that spend.
