...
- A response that comes back incomplete, whether keys are missing or the output was cut off, triggers **at most one** bounded extra request for the part that did not arrive. In normal operation this is zero extra requests.
...
The figure is an upper bound on the work. Every request a real run would make is counted, the plural-generation batches included, and the prompt is measured by serializing the payload that would actually be sent rather than by reproducing its shape in a formula, so a glossary or a tone cannot go uncounted. A real run can only send less than this, for the same two reasons the dry-run count is an upper bound: it consults the cache, and it collapses identical source strings.
