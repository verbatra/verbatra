...
Dos tipos de error viajan intactos, porque ya significan algo preciso. Un `AdapterError` que lances tú conserva su propio código, así que lanza uno (`INVALID_STRUCTURE` encaja en la mayoría de los casos) para contenido que tu formato no puede representar. Un error del sistema de archivos también conserva su código, así que un archivo ausente sigue siendo un archivo ausente.
