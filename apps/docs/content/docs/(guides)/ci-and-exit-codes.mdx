...
`translate`, `watch`, `studio`, `doctor`, and `mcp` also load `.env.local` and then `.env` from the working directory before they run, with real environment variables always winning; `check`, `diff`, `export`, `import`, `extract`, `pseudo`, `types`, and `tmx` load no `.env` files, so in CI you will normally rely on `env:` alone.
