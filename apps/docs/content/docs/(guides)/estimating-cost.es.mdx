...
- Una respuesta incompleta provoca peticiones adicionales acotadas: **como máximo una** petición de reparación cuando faltan claves, y un reintento por mitades cuando la salida se cortó. En funcionamiento normal esto son cero peticiones adicionales.
...
La cifra es un límite superior del trabajo. Se cuenta cada petición que haría una ejecución real, incluidos los lotes de generación de plurales, y el prompt se mide serializando la carga que se enviaría de verdad en lugar de reproducir su forma en una fórmula, así que ni un glosario ni un tono pueden quedar sin contar. Una ejecución real solo puede enviar menos que este plan, por las mismas dos razones por las que el recuento en seco es un límite superior: consulta la caché y agrupa las cadenas de origen idénticas. Lo único que puede hacerla enviar más es el reintento acotado de arriba, y por eso `repair requests` está en la línea de exclusiones.
