...
| Código | Significado |
| --- | --- |
| `0` | éxito: `translate` o `import` terminó cada locale completo, `check` encontró todos los locales sincronizados, `diff` no encontró cambios pendientes, `doctor` no encontró ningún problema de configuración del proyecto, `extract` completó su análisis, `export` escribió su libro, `tmx` importó o exportó su memoria de traducción, `init` preparó el proyecto, `watch`, `studio` o `mcp` se detuvo limpiamente, o se imprimió `--help` o `--version` |
...
Diez comandos aceptan `--json` para una salida legible por máquina en stdout: `translate`, `watch`, `check`, `diff`, `export`, `import`, `tmx`, `doctor`, `pseudo` y `extract`. Cada registro ocupa una línea, y cada registro va dentro del mismo envoltorio, así que ramificas sobre un solo campo y nunca tienes que adivinar de qué comando son los datos que tienes delante:
...
- **Las puertas de solo lectura no necesitan clave.** `check`, `diff`, `export`, `import`, `extract` y `tmx` nunca llaman a un proveedor, así que deja los secretos completamente fuera de esos jobs. `pseudo` tampoco llama a ninguno, y solo escribe su propia locale generada. `doctor` tampoco llama a ningún proveedor y nunca lee el valor de una clave, pero informa de una variable de clave sin definir como comprobación fallida, así que mapea la clave en cualquier job donde quieras que esa comprobación pase.

`translate`, `watch`, `studio`, `doctor` y `mcp` también cargan `.env.local` y luego `.env` desde el directorio de trabajo antes de ejecutarse, y las variables de entorno reales siempre ganan; `check`, `diff`, `export`, `import`, `extract`, `pseudo` y `tmx` no cargan archivos `.env`, así que en CI normalmente dependerás solo de `env:`.
