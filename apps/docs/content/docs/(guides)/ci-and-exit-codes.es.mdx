...
`translate`, `watch`, `studio`, `doctor` y `mcp` también cargan `.env.local` y luego `.env` desde el directorio de trabajo antes de ejecutarse, y las variables de entorno reales siempre ganan; `check`, `diff`, `export`, `import`, `extract`, `pseudo`, `types` y `tmx` no cargan archivos `.env`, así que en CI normalmente dependerás solo de `env:`.
