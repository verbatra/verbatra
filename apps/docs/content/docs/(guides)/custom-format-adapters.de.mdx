...
Der Bezeichner wandert unverändert weiter. Studios Projekt-Snapshot und das `project.snapshot`-Tool des MCP-Servers geben beide das `format`-Feld der geladenen Konfiguration wörtlich zurück, ein Projekt mit einem Drittanbieter-Adapter zeigt dort also `custom:toml` statt eines eingebauten Namens oder einer Lücke, und ein Agent, der sich orientiert, kann beides unterscheiden.
