...
`doctor` kam in 0.9.0 dazu. `pseudo` und `extract` sind die jüngsten Zugänge zu dieser Liste.
...
Die Rezepte auf dieser Seite sind die Referenz. Wenn du einem Agenten das Betriebswissen lieber vorab in die Hand gibst: das verbatra-Repository liefert drei Skill-Dokumente, die genau das tun, installierbar mit der [skills.sh](https://www.skills.sh/docs)-CLI:
