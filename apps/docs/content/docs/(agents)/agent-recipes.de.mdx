...
| [`tmx`](/docs/cli/tmx) | ja | bei `import` die gelesenen Units und pro Locale die Anzahl der hinzugefügten, behaltenen und abgelehnten Einträge; bei `export` den geschriebenen Pfad und die Anzahl der Units pro Locale |
...
| Skill | Nimm es, wenn |
| --- | --- |
| `verbatra-cli` | der Agent die `verbatra`-Binary aus einer Shell oder aus CI steuert. |
