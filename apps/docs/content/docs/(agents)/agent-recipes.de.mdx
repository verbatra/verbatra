...
`init` legt interaktiv ein Projekt an, und `studio` und `mcp` sind langlaufende Server, deshalb hat keiner der drei einen maschinenlesbaren Modus. Alles andere, was ein Agent von einem Projekt braucht, ist über die anderen zehn erreichbar.
