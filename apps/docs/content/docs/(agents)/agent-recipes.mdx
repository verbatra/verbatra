...
`doctor` joined that list in 0.9.0. `pseudo` and `extract` are the most recent additions.
...
The recipes on this page are the reference. If you would rather hand an agent the operating knowledge up front, the verbatra repository ships three skill documents that do exactly that, installable with the [skills.sh](https://www.skills.sh/docs) CLI:
