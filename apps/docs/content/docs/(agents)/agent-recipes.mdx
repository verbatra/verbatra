...
`init` is an interactive scaffolder, and `studio` and `mcp` are long-running servers, so none of the three has a machine-readable mode. Everything else an agent needs from a project is reachable from the other ten.
