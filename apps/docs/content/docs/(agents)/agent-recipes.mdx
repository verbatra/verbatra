...
| [`tmx`](/docs/cli/tmx) | yes | on `import`, the units read and the per-locale added, kept, and rejected counts; on `export`, the written path and the per-locale unit counts |
| [`pseudo`](/docs/cli/pseudo) | yes | the generated pseudolocale, its path, and the entry counts |
| [`types`](/docs/cli/types) | yes | the declaration path, the key count, how many keys take arguments, the keys whose arguments could not be determined, and whether the file was written |
| [`extract`](/docs/cli/extract) | yes | the keys added to the source locale, plus what the scan could not resolve |
| [`init`](/docs/cli/init) | no | |
| [`studio`](/docs/cli/studio) | no | |
| [`mcp`](/docs/cli/mcp) | no | |

`init` is an interactive scaffolder, and `studio` and `mcp` are long-running servers, so none of the three has a machine-readable mode. Everything else an agent needs from a project is reachable from the other eleven.

`doctor` joined that list in 0.9.0, and `tmx`, `pseudo`, `types`, and `extract` in 0.11.0.
...
Do not run this under `set -e`: a non-zero exit is the signal you came for, not a crash. Code `130` never appears here, because only the three long-running commands (`watch`, `studio` and `mcp`) can be force-stopped by a second interrupt. See [CI and exit codes](/docs/ci-and-exit-codes) for the full table.

## `watch` is a stream, not a payload

`watch --json` is different in kind from the other eight. It prints one envelope per run for the life of the process, as NDJSON: one JSON object per line, no wrapping array, no terminator. A consumer reads it line by line and keeps reading.
...
The recipes on this page are the reference. If you would rather hand an agent the operating knowledge up front, [verbatra/skills](https://github.com/verbatra/skills) holds three skill documents that do exactly that, installable with the [skills.sh](https://www.skills.sh/docs) CLI:
...
| `verbatra-studio-agent-tools` | A browser agent is driving an open Studio tab. |

They cover what an agent gets wrong without being told: which commands spend and which are free, that a dry run constructs no provider at all, what the lock file makes visible, and why a provider-spending tool can be missing from a tool list on purpose. Install only the surface you use; each document stands on its own. Swap the name after `--skill`, or repeat the flag to install more than one.
