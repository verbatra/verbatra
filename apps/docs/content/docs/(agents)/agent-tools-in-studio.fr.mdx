...
**Treize outils et aucun avis.** Sans `--allow-spend`, Studio tente treize enregistrements plutôt que quinze : les deux outils de dépense sont écartés avant même d'être tentés. Treize outils est le compte sain d'une session sans dépense, leur absence est donc l'état attendu et ne déclenche jamais d'avis.
...
La première phrase indique combien des enregistrements tentés ont échoué, et sur combien de tentatives : quinze ici, parce que cette session a été lancée avec `--allow-spend`, et treize dans une session sans ce flag. Chaque outil en échec suit ensuite sous la forme `name (ErrorName: message)`, séparés par des points-virgules. Le nom d'erreur identifie la cause : c'est le `name` de la valeur avec laquelle le navigateur a rejeté, en général une `DOMException` comme `SecurityError` ou `InvalidStateError`.
