...
- Un vrai contexte de navigation. Cela ne marche pas en headless ; l'agent pilote l'onglet ouvert et actif.
...
Ou définis la variable d'environnement de repli :
...
Avec l'option activée, Studio enregistre quinze outils, un par action du tableau de bord. Sans `--allow-spend`, il en enregistre treize : les deux outils de dépense sont entièrement omis. Les noms d'outils sont préfixés et utilisent des tirets bas (par exemple `verbatra_project_snapshot`).

### Outils de lecture (onze)
...
| `verbatra_key_integrity` | l'intégrité des placeholders, du balisage en ligne et de l'ICU pour une clé |
...
| `verbatra_locale_values` | les valeurs source et cible de chaque clé dans chaque locale cible |

### Outils d'écriture (deux)

`verbatra_translation_editEntry` modifie sur place la valeur d'une locale. Il passe par la même barrière d'intégrité que toute autre écriture verbatra : une valeur qui échoue à l'une de ses vérifications est rejetée et rien n'est écrit. Une modification acceptée écrit le fichier de locale et fait avancer l'entrée de verrou de la clé, exactement comme une modification depuis le tableau de bord. Modifier n'exige aucune option de dépense ; cela ne touche que tes fichiers locaux et n'appelle jamais de fournisseur, mais ce n'est pas sans garde-fou : l'outil est limité à 20 modifications par minute glissante, et un second appel pour la même locale et la même clé pendant qu'un autre est encore en cours est rejeté avec `ALREADY_IN_PROGRESS` (HTTP 409) plutôt que mis en file, pour que deux appelants ne se disputent pas une entrée.
...
Tous deux font passer leurs résultats par la barrière d'intégrité avant que quoi que ce soit n'atteigne le disque, et tous deux sont soumis aux limites de débit déjà en place dans Studio (retraduction à 20 par minute glissante, traduction de l'attente à 5 par minute glissante, plus un garde contre les appels concurrents). `verbatra_translation_retranslateEntry` porte en plus le même garde par entrée que la modification : un second appel concurrent pour la même locale et la même clé est rejeté avec `ALREADY_IN_PROGRESS` (HTTP 409). Sans `--allow-spend`, ils sont absents du jeu d'outils et le serveur n'enregistre même pas les points de terminaison sous-jacents, donc une dépense que tu n'as pas accordée au démarrage reste hors de portée d'un agent.

### Quand un appel échoue

Chaque outil renvoie en JSON l'enveloppe RPC propre au tableau de bord, donc un appel échoué se lit `{ "ok": false, "error": { "code": ..., "message": ... } }`. En plus des codes métier que rapporte le SDK (un rejet d'intégrité, une locale ou une clé inconnue), de `METHOD_RATE_LIMITED` pour un appel qui dépasse sa limite de débit et du refus `ALREADY_IN_PROGRESS` ci-dessus, les codes viennent de la couche RPC elle-même :

- `REQUEST_INVALID` : le corps de la requête n'était pas un objet JSON nommant une méthode, ou la réponse du serveur n'était pas une enveloppe que l'onglet pouvait lire.
- `METHOD_UNKNOWN` : la méthode n'est pas une méthode que Studio connaît, ou elle n'est pas enregistrée dans cette session, comme c'est le cas d'une méthode de dépense sans `--allow-spend`.
- `PARAMS_INVALID` : les paramètres ont échoué à la validation ; l'erreur porte une liste `issues` qui nomme chaque champ fautif.
- `SESSION_EXPIRED` : le serveur n'accepte plus le cookie de session de l'onglet, en général parce que Studio a été redémarré avec un nouveau token. L'onglet cesse d'appeler le serveur et affiche l'écran Session expired ; ouvre l'URL affichée par le Studio en cours d'exécution pour démarrer une nouvelle session.
...
**Treize outils et aucun avis.** Sans `--allow-spend`, Studio tente treize enregistrements plutôt que quinze : les deux outils de dépense sont écartés avant même d'être tentés. Treize outils, c'est le compte sain d'une session sans dépense : leur absence est donc l'état attendu et ne déclenche jamais d'avis.
...
Le décompte et le nom d'erreur sont les vrais, ceux de cette passe. L'avis n'a pas de commande de fermeture et reste affiché pendant toute la vie de la page. Recharger l'onglet lance une nouvelle passe d'enregistrement, puisque l'état n'est gardé qu'en mémoire.
...
La première phrase indique combien des enregistrements tentés ont échoué, et sur combien de tentatives : quinze ici, parce que cette session a été lancée avec `--allow-spend`, et treize dans une session sans cette option. Chaque outil en échec suit ensuite sous la forme `name (ErrorName: message)`, séparés par des points-virgules. Le nom d'erreur identifie la cause : c'est le `name` de la valeur avec laquelle le navigateur a rejeté l'enregistrement, en général une `DOMException` comme `SecurityError` ou `InvalidStateError`.
...
- [Sûreté de la traduction](/docs/translation-safety) : la barrière d'intégrité par laquelle passent chaque modification et chaque retraduction.
