...
- Find the actual locale files (commonly under `locales/`, `src/locales/`,
  ...
  - The file format: nested JSON, flat JSON, `.yml`/`.yaml`, `.xlf`/`.xliff`, `.arb`,
    ...
    gettext's `.po`/`.pot`, `.ini`, or .NET's `.resx`.
...
- Map what you found to one verbatra format id: i18next -> `i18next-json`,
  ...
  Android `strings.xml` -> `android-xml`, gettext `.po`/`.pot` -> `gettext-po`,
  `.ini` -> `ini`, .NET `.resx` -> `resx`. Plain
...
- If the format is `resx`: a `<data>` element carrying a `type` or `mimetype`
  attribute, and any entry whose name starts with `>>` or `$`, hold designer or
  binary resources and are left untouched; everything else is translated
  normally, and a target `.resx` that does not exist yet is created.
- If the format is `ini`: a key inside a `[section]` is addressed as `section.key`,
  comments and blank lines survive a write, and a new key is inserted at the end of
  its own section rather than at the end of the file.
...
`strings.xml`, gettext `.po`/`.pot`, INI, .NET `.resx`, or a project with more than
...
`strings.xml`, gettext `.po`/`.pot`, INI, and .NET `.resx` projects.
...
`doctor` runs five checks: the config, the format, the provider id, the API-key
environment variable that provider reads, and the source file. It makes no network
call and reads no key value, only the variable's name. Fix whatever it reports by editing
...
- [Die Lock-Datei](/docs/the-lock-file): die vollen Details hinter dem Sicherheitsverhalten bei der Übernahme, auf das sich dieser Prompt stützt.
