...
| [`tmx`](/docs/cli/tmx) | sí | en `import`, las unidades leídas y, por locale, los recuentos de entradas añadidas, conservadas y rechazadas; en `export`, la ruta escrita y el número de unidades por locale |
| [`pseudo`](/docs/cli/pseudo) | sí | el pseudolocale generado, su ruta y los recuentos de entradas |
| [`types`](/docs/cli/types) | sí | la ruta de la declaración, el número de claves, cuántas claves aceptan argumentos, las claves cuyos argumentos no se pudieron determinar y si el archivo se escribió |
| [`extract`](/docs/cli/extract) | sí | las claves añadidas al locale de origen y lo que el escaneo no pudo resolver |
| [`init`](/docs/cli/init) | no | |
| [`studio`](/docs/cli/studio) | no | |
| [`mcp`](/docs/cli/mcp) | no | |

`init` es un generador interactivo de andamiaje, y `studio` y `mcp` son servidores de larga duración, así que ninguno de los tres tiene modo legible por máquina. Todo lo demás que un agente necesita de un proyecto se alcanza desde los otros once.

`doctor` se incorporó a esa lista en 0.9.0, y `tmx`, `pseudo`, `types` y `extract` en 0.11.0.
...
`verbatra translate --dry-run --json` responde a "¿qué haría una ejecución?", sin llamada al proveedor, sin clave de API y sin escribir nada. Aquí va formateado para leerlo; en la salida real es una sola línea como las dos de arriba:
...
Sale con `0`. Cada lista por locale está presente aunque esté vacía, así que un agente puede acceder a ella sin comprobar antes. Una ejecución real añade un objeto `usage` a cada locale y al resumen cuando el proveedor informa de consumo; un dry run nunca lo hace, porque no hace ninguna llamada. La anatomía completa de un `LocaleSummary` está en la [referencia del SDK](/docs/sdk).
...
`ok` responde a "¿se ejecutó el comando?". `succeeded`, `partial` y `failed` responden a "¿se aplicó el trabajo?". El código de salida ya combina ambas cosas, y por eso un agente que solo lee el código de salida nunca se equivoca, y un agente que solo lee `ok` se equivoca el primer día malo de un proveedor.
...
No lo ejecutes con `set -e`: una salida distinta de cero es la señal que venías a buscar, no un fallo. El código `130` no aparece nunca aquí, porque solo los tres comandos de larga duración (`watch`, `studio` y `mcp`) pueden pararse a la fuerza con una segunda interrupción. La tabla completa está en [CI y códigos de salida](/docs/ci-and-exit-codes).

## `watch` es un flujo, no una única salida

`watch --json` es distinto en naturaleza a los otros diez. Imprime un envoltorio por ejecución durante toda la vida del proceso, como NDJSON: un objeto JSON por línea, sin array que lo envuelva y sin terminador. Un consumidor lo lee línea a línea y sigue leyendo.
...
De esa última línea se siguen dos cosas. Una ejecución fallida es un registro en el flujo, no el final de la sesión: el proceso de `watch` siguió activo y siguió traduciendo después. Y nunca cambia el código de salida, que lo decide solo la forma en que termina la sesión. Por eso un agente de larga duración trata una línea `ok: false` como un evento del que informar, no como motivo para reiniciar el proceso.

## Un bucle completo: proteger, decidir, traducir, informar

Todo junto. Esto corre desatendido, se niega a gastar en un trabajo demasiado grande para dejárselo a un robot e informa de lo que se aplicó de verdad, no de lo que se pidió:
...
## El paquete de skills

Las recetas de esta página son la referencia. Si prefieres darle a un agente el conocimiento operativo por adelantado, [verbatra/skills](https://github.com/verbatra/skills) contiene tres documentos de skill que hacen justo eso, instalables con la CLI de [skills.sh](https://www.skills.sh/docs):
...
| Skill | Úsala cuando |
| --- | --- |
| `verbatra-cli` | El agente maneja el binario `verbatra` desde una shell o desde CI. |
| `verbatra-mcp-tools` | Hay un cliente MCP conectado a `verbatra mcp`. |
| `verbatra-studio-agent-tools` | Un agente de navegador maneja una pestaña de Studio abierta. |

Cubren lo que un agente hace mal si nadie se lo dice: qué comandos gastan y cuáles son gratis, que un dry run no construye ningún proveedor, qué hace visible el archivo de bloqueo y por qué una herramienta que gasta puede faltar de una lista a propósito. Instala solo la superficie que uses; cada documento se basta a sí mismo. Cambia el nombre que va detrás de `--skill`, o repite la opción para instalar más de una.
