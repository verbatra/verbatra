...
| [`pseudo`](/docs/cli/pseudo) | sí | la pseudolocale generada, su ruta y los recuentos de entradas |
...
| [`extract`](/docs/cli/extract) | sí | las claves añadidas a la locale de origen y lo que el escaneo no pudo resolver |
| [`init`](/docs/cli/init) | no | |
| [`studio`](/docs/cli/studio) | no | |
| [`mcp`](/docs/cli/mcp) | no | |

`init` es un andamiaje interactivo, y `studio` y `mcp` son servidores de larga duración, así que ninguno de los tres tiene modo legible por máquina. Todo lo demás que un agente necesita de un proyecto se alcanza desde los otros once.
...
| `verbatra-mcp-tools` | un cliente MCP está conectado a `verbatra mcp`. |
| `verbatra-studio-agent-tools` | un agente de navegador maneja una pestaña de Studio abierta. |

Cubren lo que un agente hace mal si nadie se lo dice: qué comandos gastan y cuáles son gratis, que una ejecución en seco no construye ningún proveedor, qué hace visible el archivo de bloqueo y por qué una herramienta que gasta puede faltar de una lista a propósito. Instala solo la superficie que uses; cada documento se basta a sí mismo. Cambia el nombre que va detrás de `--skill`, o repite la opción para instalar más de una.
