...
failing that, the single configured locale the tag extends or is extended by, subtag by subtag, so
two tags whose script or region subtags differ (`zh-CN` and `zh-TW`) never match; and a tag several
locales could claim is reported rather than guessed at. Two target segments of one unit that resolve
to the same locale never let the last one win: an exact tag outranks a prefix match, and two
equal-ranked segments with different values store nothing for that locale and are counted as
`conflicting`. Every candidate translation then passes the same placeholder, ICU, degeneracy, and
emptiness gate that provider output and a filled workbook row already face, checked as the record
enters the memory rather than left to the run that later reads it.
