...
`description`, `meaning`, and `isPlural` the grouped keys share. Only up-to-date keys are compared,
values are compared after NFC normalization and trimming surrounding whitespace, and keys whose
description, meaning, plural flag, or gettext `msgctxt` differ are never grouped. It is a report
