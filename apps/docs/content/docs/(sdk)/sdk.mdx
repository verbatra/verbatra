...
- **needsReview** lists accepted, written keys the review heuristics flagged, each with its reason
  ...
  `PROVIDER_DEGRADED`, and `FUZZY_CACHE_REUSE`. The set ships as the `REVIEW_REASON_CODES` tuple,
  so build any runtime schema or exhaustive lookup from that rather than retyping the members. A
  review flag is advisory and never withholds a key, so a key never
