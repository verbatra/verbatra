...
- **budgetStanding(budget)** turns a `RunBudget` into one of three `BudgetStanding` values, the
  same classification the CLI budget line, Studio, and the `usage.summary` agent tool report:
  `"within"` when the run never reached its ceiling, `"stopped-before-ceiling"` when a `"stop"`
  budget withheld a request while `tokensUsed` was still below `maxTokens`, and `"reached"` when
  `tokensUsed` reached or passed `maxTokens`. Use it rather than reading `exceeded` alone, which is
  `true` in both of the last two cases.
