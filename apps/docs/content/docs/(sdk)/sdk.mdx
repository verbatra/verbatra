...
its `tokensUsed: 0` is not a count. Snapshots written now carry a newer `version`, which an older
Studio or agent server reports as `{ available: false }` instead of misreading their budget.
