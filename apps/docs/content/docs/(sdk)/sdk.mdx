...
- **needsReview** lists accepted, written keys the review heuristics flagged, each with its reason
  ...
  `INTEGRITY_REORDERED`, and `PROVIDER_DEGRADED`, also exported as the `REVIEW_REASON_CODES`
  tuple so you can build your own validator or exhaustive lookup from it. A review flag is advisory and never withholds a key, so a key never
