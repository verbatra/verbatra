...
translation, or `verbatra check` and Studio's per-key indicator report an inline markup mismatch
for a translation that is already on disk. In Studio, a retranslate or an edit comes back as `Rejected: inline markup mismatch`,
followed by the tags at fault: `-<a href>` for one the source had and the candidate dropped, `+<i>`
...
refuses a dropped placeholder. See
[markup parity](/docs/translation-safety#inline-markup-parity) for exactly what is compared.
...
back mis-nested, or a tag the source had beside another of the same name came back nested inside
it. Three further cases are refusals by design rather than bugs: merging two adjacent pairs into
one, changing a tag's letter case, and adding or removing an attribute name, including a `dir`
attribute for a right-to-left target. Keep the tags as the source has them and put the change
somewhere the check does not look, such as an attribute value. If the source value is the one with the broken markup, fix it there: the check stands down when
the source's own markup is not well formed, which means a broken source silently disables it for
that key.
