...
gate judges both, so a hand-typed value with matching, well-formed tags is accepted. Word order and
attribute values are never the cause, since neither is compared. If the source value is the one
with the broken markup, fix it there: the check stands down when the source's own markup is not
well formed, which means a broken source silently disables it for that key.
