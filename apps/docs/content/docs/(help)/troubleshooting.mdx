...
A refusal naming `+more than 256 inline tags` means the candidate carries more tags than the check
reads while its source does not. Three further cases are refusals by design rather than bugs:
merging two adjacent pairs into one, changing a tag's letter case, and adding or removing an
attribute name, including a `dir` attribute for a right-to-left target. Keep the tags as the source
has them and put the change somewhere the check does not look, such as an attribute value. If the
source value is the one with the broken markup, fix it there: the check stands down when the
source's own tags are malformed, which means a broken source silently disables it for that key.
