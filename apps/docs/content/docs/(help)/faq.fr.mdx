...
Quatre leviers, tous sans appel au fournisseur tant que tu n'en décides pas autrement :
...
- `--dry-run` (ou `translate({ config, dryRun: true })`) prévisualise exactement ce qui serait
  envoyé, sans appel au fournisseur et sans écriture.
...
- Gemini a une vraie offre d'API gratuite, et le fournisseur `openai-compatible` fonctionne avec un
...
locales, avec le `maxBatchSize` par défaut de 50, envoie 8 requêtes par locale, soit 24 au total.
À environ 4 caractères par token, avec des valeurs source de 40 caractères et des noms de clés de
20, cela fait à peu près 33 300 tokens d'entrée et 30 700 tokens de sortie, donc environ 64 000
tokens pour toute l'exécution. Sur `gemini-2.5-flash`, à un tarif illustratif de $0.10 par million
de tokens d'entrée et $0.40 par million de tokens de sortie, cela revient à environ 1,6 centime.

Les hypothèses derrière ce chiffre, pour que tu le remettes à l'échelle de ton projet : 400 clés
par locale et 3 locales ; des valeurs de 40 caractères et des noms de clés de 20 ; pas de
`description`, `meaning`, glossaire ni ton ; `maxBatchSize` à sa valeur par défaut ; 4 caractères
par token ; la marge d'expansion de 50 % que verbatra applique à chaque estimation de sortie. Le
tarif est une valeur d'exemple qui rend le calcul concret, pas un prix relevé : consulte le vrai sur la page tarifaire de
...
(`LENGTH_RATIO_OUTLIER`), une valeur plus longue que le budget configuré pour sa clé
(`MAX_LENGTH_EXCEEDED`), une traduction identique à la source (`EQUALS_SOURCE`), un terme de
glossaire manqué (`GLOSSARY_TERM_MISSED`), des placeholders réordonnés (`INTEGRITY_REORDERED`),
un chemin dégradé côté fournisseur (`PROVIDER_DEGRADED`), ou une valeur réutilisée depuis le
cache pour une chaîne source modifiée depuis (`FUZZY_CACHE_REUSE`). Les signalements atterrissent
sur la liste `needsReview` du résumé d'exécution et dans la file de révision de Studio. Voir
...
lancer depuis le répertoire d'un paquet trouve la configuration de ce paquet. La recherche s'arrête
au répertoire `.git` ancêtre le plus proche : elle trouve donc une configuration à la racine de ton
monorepo sans déborder sur un projet sans rapport situé au-dessus. Depuis n'importe où
...
`loadConfig`. Le `files.pattern` et le fichier de verrouillage se résolvent par rapport au répertoire
de travail.
...
quatre au `.gitignore`, et `translate`, `watch`, `import`, `tmx import` et `pseudo` complètent un `.gitignore`
existant auquel il en manque un.
...
pour `openai-compatible`. `translate`, `watch`, `doctor`, `studio` et `mcp` chargent `.env` et
...
- API key environment variable : la variable d'environnement dans laquelle ce fournisseur lit sa
  clé est définie (vérifiée seulement par son nom, jamais par sa valeur).
- Source locale file : le fichier de la locale source existe à son chemin résolu, est un fichier
  ordinaire et s'analyse correctement avec le format configuré.

Chaque vérification s'exécute même si une précédente a échoué, une exécution rapporte donc tous
les problèmes indépendants à la fois ; l'exception est la configuration elle-même : si elle échoue
au chargement, les quatre vérifications qui en ont besoin sont sautées.
...
`doctor`, `check`, `diff` et `translate --dry-run` pour prévisualiser le travail, puis s'arrête pour
...
Studio avec un agent de navigateur](/docs/agent-tools-in-studio) est encore une autre surface : une
option `--expose-agent-tools` à activer explicitement sur `verbatra studio` qui enregistre des outils WebMCP pour
...
place une étape de confirmation, ou une option `--allow-spend`, entre l'agent et un véritable appel
au fournisseur.
...
automatiquement avec verbatra lui-même, avec le format `next-intl-json`, via le fournisseur
