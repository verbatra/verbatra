...
(`LENGTH_RATIO_OUTLIER`), une valeur plus longue que le budget configuré pour sa clé (`MAX_LENGTH_EXCEEDED`), une traduction identique à la source (`EQUALS_SOURCE`), un terme de
glossaire manqué (`GLOSSARY_TERM_MISSED`), des placeholders réordonnés (`INTEGRITY_REORDERED`),
un chemin fournisseur dégradé (`PROVIDER_DEGRADED`), ou une valeur réutilisée depuis le cache pour
une chaîne source modifiée depuis (`FUZZY_CACHE_REUSE`). Les signalements atterrissent sur la liste
...
- Provider : le `provider.id` configuré se résout en une factory de fournisseur.
- API key environment variable : la variable d'environnement dont ce fournisseur lit sa clé est
  définie (vérifiée seulement par son nom, jamais par sa valeur).
- Source locale file : le fichier de la locale source existe, est un fichier normal et se parse
  sous le format configuré.
