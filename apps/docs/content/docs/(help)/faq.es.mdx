...
más largo que el presupuesto configurado para su clave (`MAX_LENGTH_EXCEEDED`), una
traducción idéntica al origen (`EQUALS_SOURCE`), un término del glosario que se perdió
(`GLOSSARY_TERM_MISSED`), marcadores de posición reordenados (`INTEGRITY_REORDERED`), una ruta de
proveedor degradada (`PROVIDER_DEGRADED`) o un valor reutilizado de la caché para una cadena de
origen que desde entonces se ha editado (`FUZZY_CACHE_REUSE`). Las señales aterrizan en la lista `needsReview` del
...
a `.gitignore`, y `translate`, `watch`, `import`, `tmx import` y `pseudo` completan un `.gitignore`
existente al que le falte alguno. Consulta
...
`doctor`, `studio` y `mcp` cargan `.env` y `.env.local` desde el directorio de trabajo (las
variables del entorno real ganan). El esquema de configuración rechaza las claves desconocidas precisamente para que un
