...
is about 33,300 input tokens and 30,700 output tokens, so about 64,000 tokens for the whole run.
On `gemini-2.5-flash`, at an illustrative rate of $0.10 per million input tokens and $0.40 per
million output tokens, that works out to about 1.6 cents.
...
glossary, or tone; `maxBatchSize` at its default; 4 characters per token; the 50 percent expansion allowance
verbatra applies to every output estimate. The rate is a placeholder that makes the arithmetic concrete, not a quoted
...
withholding them: a length far out of proportion to the source (`LENGTH_RATIO_OUTLIER`), a value
longer than the budget configured for its key (`MAX_LENGTH_EXCEEDED`), a
translation identical to the source (`EQUALS_SOURCE`), a missed glossary term
(`GLOSSARY_TERM_MISSED`), reordered placeholders (`INTEGRITY_REORDERED`), a degraded provider path
(`PROVIDER_DEGRADED`), or a value reused from the cache for a source string that has since been
edited (`FUZZY_CACHE_REUSE`). The flags land on the run summary's `needsReview` list and in Studio's
...
`translate`, `watch`, `import`, `tmx import`, and `pseudo` top up an existing `.gitignore` that is
missing any of them. See
...
`doctor`, `studio`, and `mcp` load `.env` and `.env.local` from the working directory (real
environment variables win). The config schema rejects unknown keys precisely so a secret cannot end up in a committed
...
- API key environment variable: the variable that provider reads its key from is set (checked by
  name only, never by value).
- Source locale file: the source locale file exists at its resolved path, is a regular file, and
  parses under the configured format.

Every check runs even after an earlier one fails, so one run reports every independent problem at
once; the exception is the config itself, where a failure to load skips the four checks that need
it.
