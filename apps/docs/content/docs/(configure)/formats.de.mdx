---
title: Formate
description: Die vierzehn unterstützten Formate, wie jedes seine Dateien liest und schreibt, und die Garantie der Dokumentreihenfolge der Keys.
---
verbatra arbeitet über einen Format-Adapter auf deinen Locale-Dateien. Jeder Adapter liest eine Datei in eine formatneutrale Form, auf der Vergleich, Hashing und Integritätsprüfungen laufen, und schreibt sie dann in der ursprünglichen Form der Datei zurück. Du wählst das Format über das `format`-Feld in deiner [Konfiguration](/docs/config-file). Vierzehn werden unterstützt: vier JSON-Varianten plus XLIFF, YAML, ARB, Java/Spring-Properties, Apple-`.strings`, Xcode String Catalogs (`.xcstrings`), Android `strings.xml`, gettext `.po`/`.pot`, INI und .NET-`.resx`.
...
- **vue-i18n** nutzt die Einzelklammer-Tokens `{name}` und `{0}` und leitet den Plural aus einer Pipe im Wert ab.
...
- **Inline-Markup.** Inline-Platzhalterelemente (`x`, `g`, `bx`, `ex`, `ph`, `it`, `mrk`) und Einzelklammer-`{name}`-Interpolation werden als Platzhalter extrahiert und über die Übersetzung hinweg geschützt. Beim Schreiben überleben nur diese per Allowlist zugelassenen Elemente (mit ihren eigenen minimalen, nicht ausführbaren Attributen) als lebendiges Markup in `<target>`; alles andere in einem übersetzten Wert, einschließlich eines unerwarteten Elements oder einer Namespace-Abweichung, wird stattdessen als reiner Text geschrieben.
- **Notes als Kontext.** Die `<note>` einer Trans-Unit (in 2.0 die `<notes><note>` der Unit, geteilt von jedem Segment der Unit) wird als Entwicklerkontext gelesen: Der Inhalt erreicht den Provider als Kontext zur Begriffsklärung und erscheint in der [`Context`-Spalte](/docs/manual-translation) einer exportierten Arbeitsmappe. Er wird nur gelesen und nie zurückgeschrieben.
...
- **Fortsetzungen und Escapes.** Eine Zeile, die auf einen Backslash endet, wird auf der nächsten fortgesetzt. Die Standard-Escapes (`\t`, `\n`, `\r`, `\f`, `\\`, ein maskiertes Trenn- oder Kommentarzeichen) und `\uXXXX` werden beim Lesen dekodiert. Die Eingabe wird als UTF-8 gelesen; beim Schreiben wird jedes Nicht-ASCII-Zeichen als ASCII-sicheres `\uXXXX`-Escape ausgegeben, damit die Datei auch unter einem alten ISO-8859-1-Reader lädt.
...
- **Kommentare und Escapes.** Ein `/* ... */`-Blockkommentar unmittelbar vor einem Eintrag wird als dessen Beschreibung gelesen: Er erreicht den Provider als Kontext zur Begriffsklärung und erscheint in der [`Context`-Spalte](/docs/manual-translation) einer exportierten Arbeitsmappe. Er wird nur gelesen und nie zurückgeschrieben. Ein `//`-Zeilenkommentar bleibt beim Schreiben erhalten, trägt aber keine Beschreibung. Die Escapes `\"`, `\\`, `\n`, `\t` und das vierstellige Hex-Unicode-Escape `\U` werden beim Lesen dekodiert; beim Schreiben werden Anführungszeichen, Backslash, Zeilenumbruch und Tabulator maskiert, und jedes andere Zeichen, auch Nicht-ASCII-Text, wird als rohes UTF-8 geschrieben.
- **Fehlendes Ziel.** Anders als bei XLIFF wird ein noch nicht existierendes `.strings`-Ziel aus den Einträgen synthetisiert, statt den Schreibvorgang scheitern zu lassen; das entspricht dem Verhalten des Properties-Formats.
...
- **Platzhalter sind im printf-Stil.** `%@`, `%d`, `%1$@` und das maskierte Literal `%%` werden extrahiert und über die Übersetzung hinweg geschützt, wobei Flags, Breite, Präzision und Längenmodifikatoren (`%05.2f`, `%ld`) als Dekoration gelten und nicht zur Identität des Tokens gehören. Ein positionelles Vertauschen wie `%1$@ %2$@` zu `%2$@ %1$@` wird akzeptiert, denn genau dafür gibt es positionelle Spezifizierer. Ein bloßes `%`, gefolgt von gewöhnlichem Text, wie in `"50% off"`, extrahiert nichts.
- **Plurale liegen in der begleitenden `.stringsdict`.** `Localizable.strings` bildet ein Paar mit `Localizable.stringsdict` im selben Verzeichnis; verbatra findet sie automatisch und führt ihre Plural-Kategorien in derselben Locale-Ressource zusammen, sodass die Übersetzung weiterhin genau ein `files.pattern` pro Locale anspricht. Jede in einem Eintrag vorhandene CLDR-Kategorie (`zero`, `one`, `two`, `few`, `many`, `other`) wird zu einem eigenen übersetzbaren Eintrag, mit demselben `<key>_<category>`-Suffix, das auch `i18next-json` verwendet, zum Beispiel `photo_count_one` und `photo_count_other`. Eine Locale, die nur einen Teil der Kategorien liefert (der Regelfall, da die meisten Sprachen nur `one` und `other` brauchen), übersteht den Roundtrip mit genau diesen: Für ein Ziel wird keine erfunden, und aus der Quelle wird keine verworfen. Printf-Platzhalter innerhalb eines Kategorie-Formatstrings, etwa `%d` in `"%d photos"`, werden genauso extrahiert und geschützt wie in `.strings`-Werten. Eine fehlerhafte `.stringsdict` (ungültiges XML, eine fehlende `%#@variable@`-Substitution oder eine nicht unterstützte Plural-Kategorie) wirft einen strukturierten Fehler, der Datei und Key benennt. Ein noch nicht existierendes `.stringsdict`-Ziel wird genauso angelegt wie ein `.strings`-Ziel, `.lproj`-Verzeichnis inklusive, und seine nicht übersetzbare Struktur (der Format-Key, der Substitutionsvariablenname und der Werttyp) bleibt bei einem erneuten Schreiben erhalten.
...
- **Schreibvorgänge in einen gemeinsamen Katalog laufen seriell.** Da jeder Schreibvorgang pro Locale dieselbe Datei berührt, serialisiert verbatra jede Operation, die sie schreiben kann - `translate`, eine Studio-Bearbeitung, eine Einzelkey-Neuübersetzung und den Arbeitsmappen-Import - gegeneinander. In der Praxis heißt das: Ein `apple-xcstrings`-Projekt führt diese Operationen nacheinander aus, selbst wenn `--concurrency` über 1 gesetzt ist; jedes andere Format führt seine Provider-Aufrufe weiterhin wie konfiguriert parallel aus.
- **Der Key ist der Quell-String.** Ein Katalog-Key ohne expliziten `localizations`-Eintrag für die im Dokument selbst angegebene `sourceLanguage` fällt auf den Key-Text selbst zurück, passend zu Xcodes eigener Konvention. Ein Key ohne Eintrag für irgendeine andere Locale gilt für diese Locale einfach als noch nicht übersetzt, genau wie ein fehlender Key in der Zieldatei jedes anderen Formats.
- **Plurale liegen in `variations.plural`.** Jede unter `variations.plural` einer Locale vorhandene CLDR-Kategorie (`zero`, `one`, `two`, `few`, `many`, `other`) wird zu einem eigenen übersetzbaren Eintrag, mit demselben `<key>_<category>`-Suffix, das auch `i18next-json` und die `.stringsdict`-Unterstützung des Apple-`.strings`-Formats verwenden, zum Beispiel `%lld photos_one` und `%lld photos_other`. Eine Locale, die nur einen Teil der Kategorien liefert, übersteht den Roundtrip mit genau diesen: Keine wird erfunden, keine wird verworfen.
- **Platzhalter sind im printf-Stil.** `%@`, `%d`, `%1$@`, `%lld` und das maskierte Literal `%%` werden extrahiert und über die Übersetzung hinweg geschützt, genau wie bei Apple-`.strings`-Werten; ein Längenmodifikator wie das `ll` in `%lld` ist Dekoration und nicht Teil der Identität des Tokens, also sind `%lld` und `%d` derselbe Platzhalter.
...
- **`<string>` und `<plurals>`.** Ein `<string name="key">value</string>` ist ein Eintrag. Ein `<plurals name="key">` wird zu einem eigenen Eintrag pro `<item quantity="...">` (`zero`, `one`, `two`, `few`, `many`, `other`), mit dem Key `key[quantity]`, zum Beispiel `count[one]` und `count[other]`. Eine Locale, die nur einen Teil der Quantitäten liefert, übersteht den Roundtrip mit genau diesen: Keine wird erfunden, keine wird verworfen. Jeder Ressourcenname `name` wird gegen Androids Bezeichnergrammatik geprüft (ein Buchstabe oder Unterstrich, dann Buchstaben, Ziffern oder Unterstriche), bevor er zum Key wird, sodass ein Name keine `[quantity]`-förmige Kollision mit dem echten Plural-Key-Raum vortäuschen kann.
...
- **Escaping wird beim Lesen dekodiert, beim Schreiben neu kodiert.** `\'`, `\"`, `\n`, `\t` und ein führendes `\@` oder `\?` werden beim Lesen zu ihren wörtlichen Zeichen dekodiert, und dieselben Zeichen (plus ein führendes, nicht maskiertes `@` oder `?`) werden beim Schreiben wieder maskiert; XMLs eigene Entities `&`, `<` und `>` werden unabhängig davon von der XML-Schicht darunter behandelt, sodass ein übersetzter Wert mit einem dieser Zeichen sicher zurückgeschrieben wird, ohne dass du selbst maskieren musst.
- **Platzhalter sind im printf-Stil.** `%s`, `%d`, `%1$s` und das maskierte Literal `%%` werden extrahiert und über die Übersetzung hinweg geschützt. Ein bloßes `%` gefolgt von gewöhnlichem Text, wie in `"50% off"`, extrahiert nichts: Der Extraktor behandelt ein Leerzeichen nicht als gültiges Konvertierungsflag, anders als Javas eigenes `String.format`, sodass ein Prozentzeichen in gewöhnlichem Fließtext nie für einen Platzhalter gehalten wird.
...
Das `gettext-po`-Format deckt GNU gettexts `.po`- und `.pot`-Kataloge ab, erkannt an einer der beiden Endungen: eine flache Folge von `msgid`/`msgstr`-Einträgen, mit der `msgid` als Key, nie in einen Baum aufgespalten.

- **`msgctxt`-Disambiguierung.** Ein `msgctxt` vor einem Eintrag setzt sich mit dessen `msgid` zu einem Key zusammen, sodass zwei Einträge mit derselben `msgid` unter verschiedenen Kontexten nie kollidieren. Der zusammengesetzte Key nutzt einen reservierten Private-Use-Codepoint (U+E000) als internen Trenner, ein Zeichen, das keine echte `.po`-Datei legitim enthält; ein Quelltext, der es doch enthält, wird mit einem strukturierten Fehler abgelehnt statt den Key still zu beschädigen.
- **Plurale sind `msgid_plural`/`msgstr[n]`, adressiert über den Index, nicht über die CLDR-Kategorie.** Jedes `msgstr[n]` wird zu einem eigenen Eintrag, mit dem Key `key[n]` (`n` der rohe Dezimalindex), zum Beispiel `one item[0]` und `one item[1]`. Das ist bewusst nicht die `<key>_<category>`-Suffixkonvention, die die JSON-Formate und Apples `.stringsdict` verwenden: Einen gettext-Index in eine CLDR-Kategorie umzuwandeln, würde bedeuten, den `Plural-Forms`-C-Ausdruck der Datei selbst auszuwerten, was verbatra nicht tut. Der `Plural-Forms`-Header wird wörtlich als unangetasteter Text erhalten; verbatra liest daraus nur die Ganzzahl `nplurals=N`, um jeden `msgstr[n]`-Index auf Gültigkeit zu prüfen. Eine Datei mit Plural-Einträgen und ohne `Plural-Forms`-Header wird abgelehnt, genau wie `msgfmt --check` es selbst durchsetzt.
...
Das `ini`-Format deckt klassische INI-Konfigurationsdateien ab, erkannt an der Endung `.ini`: eine Ebene von `[section]`-Headern über `key=value`-Zeilen, nie in einen tieferen Baum aufgespalten.

- **Eine Section wird zum ersten Key-Segment.** Ein Key innerhalb von `[home]` wird als `home.title` adressiert; ein Key oberhalb des ersten Section-Headers behält seinen blanken Namen. Ein wörtlicher Punkt in einem Section- oder Key-Namen wird im zusammengesetzten Key maskiert, sodass ein `a.b` auf oberster Ebene und ein `b` innerhalb von `[a]` zwei verschiedene Einträge bleiben.
- **Kommentare, Leerzeilen und Abstände überleben einen Schreibvorgang.** Ein Schreibvorgang liest die Zieldatei erneut und ersetzt nur den Wert jeder Zeile, zu der es einen Eintrag gibt; Key-Schreibweise, Leerraum um das `=`, die `;`- und `#`-Kommentare und die Leerzeilen bleiben genau so, wie sie waren. Ein Key, den die Datei noch nicht hat, wird am Ende seines eigenen Section-Blocks eingefügt statt am Dateiende, und eine noch nicht vorhandene Section wird am Ende angelegt.
- **Werte sind Klartext, optional in Anführungszeichen.** Ein in doppelte Anführungszeichen gesetzter Wert wird beim Lesen entpackt, wobei `\\`, `\"`, `\n`, `\r` und `\t` dekodiert werden. Ein Wert, der sonst führenden oder abschließenden Leerraum verlieren würde oder einen Zeilenumbruch enthält, wird in Anführungszeichen zurückgeschrieben. Ein `;` mitten in der Zeile gehört zum Wert und ist kein Kommentar.
- **Platzhalter sind Tokens mit einfachen geschweiften Klammern.** INI hat keine eigene Platzhalterkonvention, deshalb schützt verbatra `{name}` und `{0}`, die Syntax, auf die INI-basierte Setups konvergieren. Alles andere im Wert ist gewöhnlicher Text.
- **Keine Plurale.** Das Format kennt kein Plural-Konstrukt, also ist jeder Eintrag ein einzelner String.
- **Fehlerhafte Eingaben sind strukturiert.** Ein Section-Header darf einen angehängten `;`- oder `#`-Kommentar tragen. Ein unterminierter oder leerer `[section]`-Header, oder alles nach der schließenden Klammer, was kein Kommentar ist, löst einen strukturierten `AdapterError` aus statt eines rohen Parse-Fehlers. Eine Zeile ohne `=` bleibt wörtlich erhalten und wird nie zu einem Eintrag.
- **Fehlende Zieldatei.** Eine noch nicht existierende `.ini`-Zieldatei wird aus den Einträgen heraus angelegt, statt den Schreibvorgang scheitern zu lassen.

## .NET .resx

<AvailableFrom version="0.11.0" />

Das `resx`-Format deckt die XML-Ressourcendateien von .NET ab, erkannt an der Endung `.resx`: eine flache Liste von `<data name="Key"><value>text</value></data>`-Elementen unter einem `<root>`, nie in einen Baum aufgespalten. Es beansprucht ausschließlich `.resx`, sodass ein Android-`strings.xml` weiterhin eindeutig zu `android-xml` aufgelöst wird.

- **Die Schema-Präambel und die Header werden nie angefasst.** Ein Schreibvorgang liest das Zieldokument erneut und ersetzt nur das `<value>` der Einträge, die er hat; der eingebettete XSD-Schemablock, jedes `<resheader>`, jedes `<metadata>`- und `<assembly>`-Element und jeder Eintrag, für den es keine Übersetzung gibt, kommen unverändert zurück, auf dem Zeilenumbruch der Datei selbst. Das Einzige, was ein Schreibvorgang nicht wiederherstellt, ist ein führendes Byte Order Mark, das beim Lesen entfernt wird.
- **Nicht-String- und Designer-Einträge bleiben erhalten und werden nicht übersetzt.** Ein `<data>`-Element mit einem `type`- oder `mimetype`-Attribut enthält ein serialisiertes Objekt (ein Bild, ein Icon, ein Byte-Array) und wird unangetastet durchgereicht. Ebenso jeder Eintrag, dessen Name mit `>>` oder `$` beginnt: das sind Designer-Metadaten von Visual Studio und kein für Nutzer sichtbarer Text. Keiner davon wird zu einem übersetzbaren Eintrag, und keiner wird neu geschrieben.
- **Ein `<comment>` wird zur Beschreibung des Eintrags.** Er erreicht den Provider als Kontext zur Begriffsklärung und erscheint in der [`Context`-Spalte](/docs/manual-translation) einer exportierten Arbeitsmappe. Zurückgeschrieben wird er nie.
- **Platzhalter sind .NET-Composite-Format-Items.** `{0}`, `{1,-10}` und `{2:C}` werden vollständig extrahiert, samt Ausrichtung und Formatangabe, sodass eine Übersetzung, die `{0:C}` verliert oder zu `{0}` verflacht, die Integritätsprüfung nicht besteht. Weil hier die Platzhalter-Integrität der einzige Schutz ist, werden auch die Klammern geschützt: ein verdoppeltes `{{` oder `}}` und eine Klammer, die zu keinem Format-Item gehört, sind eigene Tokens. So wird ein zu `{` entmaskiertes `{{` oder eine erfundene einzelne Klammer erkannt, statt in deiner App als `FormatException` aufzuschlagen.
- **Bewusst keine ICU-Validierung.** Composite Formatting ist kein ICU, und verbatras Kanal für Nachrichtengültigkeit meldet ausdrücklich ICU; `.resx`-Werte werden deshalb vollständig über die Platzhalter-Integrität geschützt statt über eine Syntaxprüfung, die den Fehler falsch benennen würde.
- **Keine Plurale.** `.resx` kennt kein Plural-Konstrukt, also ist jeder Eintrag ein einzelner String.
- **Fehlende Zieldatei.** Eine noch nicht existierende `.resx`-Zieldatei wird mit den Standard-Headern `resmimetype`, `version`, `reader` und `writer` erzeugt, sodass das Ergebnis so, wie es ist, unter `ResXResourceReader` lädt.
- **Ein Name, den das Ziel nicht tragen kann, wird abgelehnt, nie still übersprungen.** Hält die Zieldatei einen Key bereits als typisierte Ressource, als Designer-Metadatum oder mit Markup in seinem `<value>`, kann der übersetzte Wert dort nicht hin. Der Schreibvorgang scheitert mit einem strukturierten `AdapterError`, der den Key benennt, statt Erfolg zu melden und den Wert zu verwerfen. Ein Key, der nie zurückgelesen werden könnte, wird genauso abgelehnt, auch wenn das Ziel unter diesem Namen nichts hält: ein leerer Key, oder einer, der mit `>>` oder `$` beginnt, käme beim nächsten Lesen als Designer-Metadatum zurück statt als Text.
- **Fehlerhafte Eingaben sind strukturiert.** Ein Dokument, das kein gültiges XML ist, dessen Wurzelelement nicht `<root>` ist oder das eine DTD oder eine Entity deklariert, löst einen strukturierten `AdapterError` aus.
...
## Formate von außerhalb verbatra

<AvailableFrom version="0.11.0" />

Die vierzehn Formate oben sind die, die verbatra mitbringt. Ein Format, das es nicht mitbringt, kannst du über einen Adapter in deinem eigenen Paket liefern: Du benennst es mit einem `custom:`-Bezeichner wie `custom:toml` und übergibst es verbatra als Teil einer Registry. Die eingebaute Menge bleibt geschlossen, und kein eingebauter Name enthält einen Doppelpunkt, ein Bezeichner von außen kann also nie einen eingebauten überdecken.

Das ist ein programmatischer Weg über `@verbatra/sdk`, kein Weg über die Kommandozeile: Das `verbatra`-Binary lädt von sich aus kein Plugin. Ein Adapter, den du installierst, ist voll vertrauenswürdiger Code auf dem Vertrauensniveau jeder anderen Abhängigkeit, und verbatra kapselt ihn nicht ab. Wie du einen baust und worauf du dich beim Installieren einlässt, steht unter [Eigene Format-Adapter](/docs/custom-format-adapters).

## Inline-Markup in Werten

<AvailableFrom version="0.11.0" />

Ein Wert kann eigenes Inline-HTML oder -XML tragen: `<b>` in einem i18next-`Trans`-Block oder einem
gettext-Katalog, `<a href>` in einer Vue-I18n- oder ngx-translate-Message, ein maskiertes
`&lt;b&gt;` in einem Android-`<string>`. Dieses Markup ist in keinem dieser Formate ein
Platzhalter, deshalb vergleicht die
[Markup-Paritätsprüfung](/docs/translation-safety#inline-markup-parität) es getrennt und lehnt eine
Übersetzung ab, die ein Tag weglässt, erfindet, umbenennt oder aus dem Gleichgewicht bringt.

Sie gilt für jedes Format und tritt pro Tag statt pro Wert zurück: Ein Tag, das der Adapter ohnehin
als Platzhalter meldet, bleibt der Platzhalterprüfung überlassen, während jedes andere Tag im
selben Wert weiterhin verglichen wird.

- `xliff` extrahiert seine Inline-Platzhalterelemente (`x`, `g`, `bx`, `ex`, `ph`, `it`, `mrk`)
  selbst, sie sind also von der Platzhalterprüfung abgedeckt und werden nie doppelt gezählt.
  Einfaches HTML, das XLIFF nicht tokenisiert, etwa ein `<b>`-Paar, wird weiterhin als Markup
  verglichen, auch wenn es im selben Wert neben einem Inline-Element steht.
- `next-intl-json` und `arb` parsen ICU-Rich-Text-Tags, ein gepaartes `<b>...</b>` ist für sie also
  bereits ein Platzhalter, ein selbstschließendes `<br/>` im selben Wert dagegen nicht und wird als
  Markup verglichen. Ein Tag mit Attributen ist überhaupt kein gültiges ICU, und diese Formate
  lehnen es unabhängig von der Markup-Parität als ungültige Message-Syntax ab.
- `android-xml`-Werte erreichen die Prüfung mit aufgelöster Maskierung, ein `&lt;b&gt;` auf der
  Platte wird also als `<b>` verglichen. Ein `<string>`, das echte Inline-Elemente enthält, wird
  durchgereicht und wie oben beschrieben gar nicht erst übersetzt.
- Jedes andere Format hat kein eigenes Markup-Konzept, die Markup-Parität ist dort also die einzige
  Prüfung, die es sieht.

## Warum das Format wichtig ist

Das Format sagt verbatra zwei Dinge, die es für einen sicheren Lauf braucht. Erstens die Platzhaltersyntax, damit die [Platzhalter-Integritätsprüfung](/docs/translation-safety) weiß, was sie vor und nach der Übersetzung vergleichen soll. Zweitens, für die ICU-Formate, welche Werte zu validieren sind, damit ein Quell-Key mit ungültigem ICU übersprungen statt in kaputtem Zustand gesendet wird. Das falsche Format vergleicht die falschen Tokens, wähle es also passend zu deiner i18n-Bibliothek: `i18next-json` für i18next, `vue-i18n-json` für vue-i18n, `next-intl-json` für next-intl, `ngx-translate-json` für ngx-translate, `xliff` für XLIFF-Dateien, `yaml` für YAML-basierte i18n, `arb` für Flutter, `properties` für Java- oder Spring-`.properties`-Dateien, `apple-strings` für Apple-`.strings`-Dateien, `apple-xcstrings` für Xcode-String-Catalog-`.xcstrings`-Dateien, `android-xml` für Android `strings.xml`, `gettext-po` für gettext-`.po`/`.pot`-Kataloge, `ini` für INI-Konfigurationsdateien und `resx` für .NET-Ressourcendateien.
