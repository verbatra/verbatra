...
- **Ein Name, den das Ziel nicht tragen kann, wird abgelehnt, nie still übersprungen.** Hält die Zieldatei einen Key bereits als typisierte Ressource, als Designer-Metadatum oder mit Markup in seinem `<value>`, kann der übersetzte Wert dort nicht hin. Der Schreibvorgang scheitert mit einem strukturierten `AdapterError`, der den Key benennt, statt Erfolg zu melden und den Wert zu verwerfen.
