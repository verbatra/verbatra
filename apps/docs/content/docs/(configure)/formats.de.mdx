...
- **Eine Section wird zum ersten Key-Segment.** Ein Key innerhalb von `[home]` wird als `home.title` adressiert; ein Key oberhalb des ersten Section-Headers behält seinen blanken Namen. Ein wörtlicher Punkt in einem Section- oder Key-Namen wird im zusammengesetzten Key escapt, sodass ein `a.b` auf oberster Ebene und ein `b` innerhalb von `[a]` zwei verschiedene Einträge bleiben.
...
- **Keine Plurale.** Das Format kennt kein Plural-Konstrukt, also ist jeder Eintrag eine einzelne Zeichenkette.
...
- **Ein `<comment>` wird zur Beschreibung des Eintrags.** Er erreicht den Provider als Kontext zur Disambiguierung und erscheint in der [`Kontext`-Spalte](/docs/manual-translation) einer exportierten Arbeitsmappe. Zurückgeschrieben wird er nie.
- **Platzhalter sind .NET-Composite-Format-Items.** `{0}`, `{1,-10}` und `{2:C}` werden vollständig extrahiert, samt Ausrichtung und Formatangabe, sodass eine Übersetzung, die `{0:C}` verliert oder zu `{0}` verflacht, die Integritätsprüfung nicht besteht. Weil hier die Platzhalter-Integrität der einzige Schutz ist, werden auch die Klammern geschützt: ein verdoppeltes `{{` oder `}}` und eine Klammer, die zu keinem Format-Item gehört, sind eigene Tokens. So wird ein zu `{` entschärftes `{{` oder eine erfundene einzelne Klammer erkannt, statt in deiner App als `FormatException` aufzuschlagen.
...
- **Keine Plurale.** `.resx` kennt kein Plural-Konstrukt, also ist jeder Eintrag eine einzelne Zeichenkette.
