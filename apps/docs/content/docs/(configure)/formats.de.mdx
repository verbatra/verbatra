...
- **Fehlerhafte Eingaben sind strukturiert.** Ein unterminierter oder leerer `[section]`-Header löst einen strukturierten `AdapterError` aus statt eines rohen Parse-Fehlers. Eine Zeile ohne `=` bleibt wörtlich erhalten und wird nie zu einem Eintrag.
...
- **Die Schema-Präambel und die Header werden nie angefasst.** Ein Schreibvorgang liest das Zieldokument erneut und ersetzt nur das `<value>` der Einträge, die er hat; der eingebettete XSD-Schemablock, jedes `<resheader>` und die eigene Formatierung des Dokuments kommen byteidentisch zurück.
...
- **Platzhalter sind .NET-Composite-Format-Items.** `{0}`, `{1,-10}` und `{2:C}` werden vollständig extrahiert, samt Ausrichtung und Formatangabe, sodass eine Übersetzung, die `{0:C}` verliert oder zu `{0}` verflacht, die Integritätsprüfung nicht besteht. Die verdoppelten `{{` und `}}`, die für eine wörtliche Klammer stehen, werden als Literale gelesen, nicht als Platzhalter.
