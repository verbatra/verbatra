...
The `ini` format covers classic INI configuration files, detected by the `.ini` extension: one level of `[section]` headers over `key=value` lines, never split into a deeper tree.

- **A section becomes the first key segment.** A key inside `[home]` is addressed as `home.title`; a key above the first section header keeps its bare name. A literal dot in a section or key name is escaped inside the composed key, so a top-level `a.b` and a `b` inside `[a]` stay two distinct entries.
- **Comments, blank lines, and spacing survive a write.** A write re-reads the destination and rewrites only the value of each line it has an entry for, leaving the key spelling, the whitespace around `=`, the `;` and `#` comments, and the blank lines exactly as they were. A key the file does not have yet is inserted at the end of its own section block rather than at the end of the file, and a section the file does not have yet is created at the end.
- **Values are plain text, optionally quoted.** A value wrapped in double quotes is unquoted on read, with `\\`, `\"`, `\n`, `\r`, and `\t` decoded. A value that would otherwise lose leading or trailing space, or that carries a line break, is written back quoted. An inline `;` is part of the value, not a comment.
- **Placeholders are single-brace tokens.** INI has no placeholder convention of its own, so verbatra guards `{name}` and `{0}`, the syntax INI-based setups converge on. Anything else in the value is ordinary text.
- **No plurals.** The format has no plural construct, so every entry is a single string.
- **Malformed input is structured.** A section header may carry a trailing `;` or `#` comment. An unterminated or empty `[section]` header, or anything after the closing bracket that is not a comment, raises a structured `AdapterError` rather than a raw parse failure. A line with no `=` is kept verbatim and never becomes an entry.
- **Missing destination.** A target `.ini` that does not exist yet is created from the entries rather than failing the write.

## .NET .resx

<AvailableFrom version="0.11.0" />

The `resx` format covers .NET XML resource files, detected by the `.resx` extension: a flat list of `<data name="Key"><value>text</value></data>` elements under a `<root>`, never split into a tree. It claims `.resx` only, so an Android `strings.xml` still resolves to `android-xml` with no ambiguity.

- **The schema preamble and the headers are never touched.** A write re-reads the destination document and patches only the `<value>` of the entries it has, so the inline XSD schema block, every `<resheader>`, every `<metadata>` and `<assembly>` element, and every entry it has no translation for come back unchanged, on the file's own line terminator. The one thing a write does not restore is a leading byte order mark, which is dropped when the file is read.
- **Non-string and designer entries are preserved, not translated.** A `<data>` element carrying a `type` or `mimetype` attribute holds a serialized object (an image, an icon, a byte array) and is read through untouched. So is any entry whose name starts with `>>` or `$`, which is Visual Studio designer metadata rather than user-facing text. None of these become translatable entries, and none of them are rewritten.
- **A `<comment>` becomes the entry description.** It reaches the provider as disambiguation context and appears in the [`Context` column](/docs/manual-translation) of an exported workbook. It is never written back.
- **Placeholders are .NET composite format items.** `{0}`, `{1,-10}`, and `{2:C}` are extracted whole, alignment and format specifier included, so a translation that drops `{0:C}` or flattens it to `{0}` fails the integrity check. Because placeholder integrity is the only guard here, braces are guarded too: a doubled `{{` or `}}`, and a brace belonging to no format item, are tokens of their own, so unescaping `{{` into `{` or inventing a stray brace is caught rather than reaching a `FormatException` in your app.
- **No ICU validation, by choice.** Composite formatting is not ICU, and verbatra's message-validity channel reports ICU specifically, so `.resx` values are guarded entirely through placeholder integrity rather than through a message-syntax check that would mislabel the failure.
- **No plurals.** `.resx` has no plural construct, so every entry is a single string.
- **Missing destination.** A target `.resx` that does not exist yet is synthesized with the standard `resmimetype`, `version`, `reader`, and `writer` headers, so the result loads under `ResXResourceReader` as it stands.
- **A name the destination cannot carry is refused, never silently skipped.** If the target file already holds a key as a typed resource, as designer metadata, or with markup inside its `<value>`, the translated value cannot go there. The write fails with a structured `AdapterError` naming the key, rather than reporting success and dropping the value. A key that could never be read back is refused the same way, even when the target holds nothing under that name: an empty key, or one starting with `>>` or `$`, would come back as designer metadata rather than as text on the next read.
- **Malformed input is structured.** A document that is not valid XML, whose root element is not `<root>`, or that declares a DTD or an entity, raises a structured `AdapterError`.
...
## Formats from outside verbatra

<AvailableFrom version="0.11.0" />

The fourteen formats above are the ones verbatra ships. A format it does not ship can be supplied by an adapter in your own package, named with a `custom:` identifier such as `custom:toml` and handed to verbatra as part of a registry. The built-in set stays closed, and no built-in name contains a colon, so a third-party identifier can never shadow one.

This is a programmatic path through `@verbatra/sdk`, not a command-line one: the `verbatra` binary loads no plugin of its own. An adapter you install is fully trusted code, at the trust level of any other dependency, and verbatra does not sandbox it. See [Custom format adapters](/docs/custom-format-adapters) for how to build one and for what you are trusting when you install one.

## Inline markup in values

<AvailableFrom version="0.11.0" />

A value may carry inline HTML or XML of its own: `<b>` in an i18next `Trans` block or a gettext
catalog, `<a href>` in a Vue I18n or ngx-translate message, an escaped `&lt;b&gt;` inside an
Android `<string>`. This markup is not a placeholder in any of these formats, so the
[markup parity check](/docs/translation-safety#inline-markup-parity) compares it separately and
refuses a translation that drops, invents, renames or unbalances a tag.

It applies to every format, and it stands down per tag rather than per value: a tag the adapter
already reports as a placeholder is left to the placeholder check, while every other tag in the
same value is still compared.

- `xliff` extracts its inline placeholder elements (`x`, `g`, `bx`, `ex`, `ph`, `it`, `mrk`)
  itself, so those are guarded by the placeholder check and never counted twice. Plain HTML that
  XLIFF does not tokenise, such as a `<b>` pair, is still compared as markup, including when it
  sits in the same value as an inline element.
- `next-intl-json` and `arb` parse ICU rich-text tags, so a paired `<b>...</b>` is already a
  placeholder for them, while a self-closing `<br/>` in the same value is not and is compared as
  markup. An attribute-bearing tag is not valid ICU at all and these formats refuse it as invalid
  message syntax, independently of markup parity.
- `android-xml` values reach the check with their escaping decoded, so `&lt;b&gt;` on disk is
  compared as `<b>`. A `<string>` holding live inline elements is read-through and never
  translated at all, as described above.
- Every other format has no markup concept of its own, so markup parity is the only check that
  sees it.

## Why the format matters

The format tells verbatra two things it needs for a safe run. First, the placeholder syntax, so the [placeholder integrity check](/docs/translation-safety) knows what to compare before and after translation. Second, for the ICU formats, which values to validate, so an invalid ICU source key is skipped rather than sent in a broken state. The wrong format compares the wrong tokens, so match it to your i18n library: `i18next-json` for i18next, `vue-i18n-json` for vue-i18n, `next-intl-json` for next-intl, `ngx-translate-json` for ngx-translate, `xliff` for XLIFF files, `yaml` for YAML-based i18n, `arb` for Flutter, `properties` for Java or Spring `.properties` files, `apple-strings` for Apple `.strings` files, `apple-xcstrings` for Xcode String Catalog `.xcstrings` files, `android-xml` for Android `strings.xml`, `gettext-po` for gettext `.po`/`.pot` catalogs, `ini` for INI configuration files, and `resx` for .NET resource files.
