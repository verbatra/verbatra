...
- **Malformed input is structured.** An unterminated or empty `[section]` header raises a structured `AdapterError` rather than a raw parse failure. A line with no `=` is kept verbatim and never becomes an entry.
...
- **The schema preamble and the headers are never touched.** A write re-reads the destination document and patches only the `<value>` of the entries it has, so the inline XSD schema block, every `<resheader>`, and the document's own formatting come back byte-identical.
...
- **Placeholders are .NET composite format items.** `{0}`, `{1,-10}`, and `{2:C}` are extracted whole, alignment and format specifier included, so a translation that drops `{0:C}` or flattens it to `{0}` fails the integrity check. The doubled `{{` and `}}` that stand for a literal brace are read as literals, not as placeholders.
