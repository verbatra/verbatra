...
It applies to every format, but per value rather than per format, and it stands down for a value
whose own adapter already reports its markup as placeholders:
...
- `xliff` extracts its inline placeholder elements (`x`, `g`, `bx`, `ex`, `ph`, `it`, `mrk`)
  ...
  XLIFF does not tokenise, such as a `<b>` pair, is still compared as markup.
- `next-intl-json` and `arb` parse ICU rich-text tags, so a paired `<b>...</b>` is already a
  placeholder for them. An attribute-bearing tag is not valid ICU at all and these formats refuse
  it as invalid message syntax, independently of markup parity.
