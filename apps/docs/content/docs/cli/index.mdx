---
title: Overview
description: "The verbatra command: its twelve subcommands, the shared flags, how it reads your environment, and the exit-code contract."
---
`@verbatra/cli` ships the `verbatra` binary, a thin wrapper over [`@verbatra/sdk`](/docs/sdk). Twelve commands do the work:
...
- `--json` prints a machine-readable [envelope](/docs/ci-and-exit-codes#json-output) on stdout, one line per record, keeping stdout clean for piping.
  ...
  - `extract`, `translate`, `watch`, `check`, `diff`, `doctor`, `pseudo`, `export`, and `import` support it; `init`, `studio`, and `mcp` do not.
