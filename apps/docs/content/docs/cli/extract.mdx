...
| Reported as | When |
| --- | --- |
| `dynamic` | the key argument is not one complete static string: a variable, a member expression, a template literal carrying an expression, a concatenation, or a namespace-qualified key. It is never written with a guessed or truncated key. |
