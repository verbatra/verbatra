...
| `diagnostics` | a file or directory could not be read all the way: it vanished, it is above the size limit, the scan threw on it, an unterminated block comment or template literal abandoned the rest of it, or a quoted string ran into the end of its line. Whatever was read before that point is still reported. |
