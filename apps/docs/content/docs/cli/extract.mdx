...
The `i18next` framework reads the `t(...)`, `$t(...)`, and `<object>.t(...)` call shapes across `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.mts`, and `.cts` files. The key is the first argument. A default value is either the second argument or a `defaultValue` field on the options object:
...
- A key in the catalog that no call site mentions is left alone. `extract` adds and reports; it never deletes.
...
| Reported as | When |
| --- | --- |
| `dynamic` | the key argument is not a static string: a variable, a member expression, or a template literal carrying an expression. It is never written with a guessed key. |
...
| `diagnostics` | a file or directory could not be read: it vanished, it is above the size limit, or the scan threw on it. |
