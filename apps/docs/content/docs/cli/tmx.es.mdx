...
2. Si no, la etiqueta solo puede llegar a un locale configurado que ella amplía o que la amplía a ella, subetiqueta a subetiqueta. Un archivo escrito en `en-US` aterriza así en un proyecto configurado con `en`, y un archivo escrito en `pt` aterriza en un proyecto configurado solo con `pt-BR`, siempre que haya exactamente un locale configurado que encaje.
3. Dos etiquetas cuyas subetiquetas de escritura o de región difieren nunca coinciden. `zh-CN` nunca se guarda como `zh-TW`, `sr-Latn` nunca como `sr-Cyrl` y `de-AT` nunca como `de-CH`: esa etiqueta se informa como sin coincidencia con ningún locale configurado, no se adivina.
4. Si dos locales configurados pudieran reclamar la etiqueta por igual, no se elige ninguno. `pt` en un proyecto con `pt-BR` y `pt-AO` se informa como ambiguo, no se adjudica al que llegó primero.
