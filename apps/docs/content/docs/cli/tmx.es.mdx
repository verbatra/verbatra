...
Vale la pena decir una consecuencia con claridad, porque ninguna comprobación en el momento del import puede vigilarla. El texto de origen de una unidad aceptada se escribe en el índice de orígenes de la memoria, que es justo contra lo que la [reutilización difusa](/docs/the-cache) puntúa una cadena modificada. Un origen importado que solo se parece a una cadena de tu proyecto puede por tanto servirse para esa cadena por reutilización difusa, aunque el camino exacto nunca la emparejaría: el camino exacto se indexa por un hash que cubre la descripción, el significado y la marca de plural, y una unidad TMX no lleva ninguno de los tres, mientras que la reutilización difusa compara solo textos de origen.
