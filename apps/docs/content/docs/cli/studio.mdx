...
Open that exact URL. The token is accepted only as `?token=` on that first page load: the server checks it, sets an HttpOnly session cookie, and redirects to `/`. Every later request is checked against the cookie, and one without a valid cookie is rejected. The token is not single-use: visiting the printed URL again, in another tab or another browser, sets the cookie again, so treat the URL as a secret for as long as Studio runs. Nothing is printed before the port is successfully bound, so a failed startup never leaks a URL or a token.

A failed bind is reported on stderr as `verbatra: error [<code>] <message>` with one of two codes: `PORT_IN_USE` when another process already holds the port (pick another with `--port`), or `BIND_FAILED` when the socket came up on an address other than `127.0.0.1`, which Studio refuses to serve on.
