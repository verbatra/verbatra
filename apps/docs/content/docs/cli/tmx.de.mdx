---
title: verbatra tmx
description: Importiere einen TMX-Übersetzungsspeicher aus einem anderen Werkzeug oder exportiere den Übersetzungsspeicher dieses Projekts als TMX.
status: new
---
<AvailableFrom version="0.11.0" />

TMX ist das Austauschformat, das die gängigen Übersetzungsplattformen erzeugen und lesen. `verbatra tmx import` bringt einen Übersetzungsspeicher, den eine davon exportiert hat, in [den Übersetzungsspeicher dieses Projekts](/docs/the-cache), sodass ein späterer Lauf diese Strings wiederverwendet, statt erneut dafür zu bezahlen. `verbatra tmx export` schreibt den Übersetzungsspeicher dieses Projekts im selben Format wieder heraus, damit die Arbeit genauso leicht wieder gehen kann, wie sie gekommen ist.

Keine der beiden Richtungen ruft einen Provider auf, liest einen API-Key oder stellt eine Netzwerkanfrage. Jeder Wert kommt aus der Datei oder aus dem Übersetzungsspeicher, der ohnehin schon auf der Platte liegt.
...
| --- | --- | --- | --- |
| `--cwd` | `<path>` | aktuelles Verzeichnis | Konfiguration und Übersetzungsspeicher von diesem Verzeichnis aus auflösen |
...
| `--dry-run` | keins | aus | nur beim Import, beim Export abgelehnt: prüfen und melden, ohne den Übersetzungsspeicher zu verändern |
| `--overwrite` | keins | aus | nur beim Import, beim Export abgelehnt: eine importierte Einheit eine Übersetzung ersetzen lassen, die der Übersetzungsspeicher schon hat |
...
Eine Datei aus einem anderen Werkzeug ist nicht vertrauenswürdige Eingabe, und eine Einheit, die es in den Übersetzungsspeicher schafft, kann wiederverwendet werden, ohne dass jemand noch einmal gefragt wird. Also wird nichts gespeichert, bevor es sich das verdient hat. Jeder Übersetzungskandidat durchläuft dasselbe Integritäts-Gate wie [Provider-Ausgabe und eine ausgefüllte Übersetzer-Übergabe](/docs/translation-safety), geprüft beim Eintritt in den Übersetzungsspeicher, statt dem Lauf überlassen zu werden, der sie später liest:
...
- sie darf nicht in ausufernde Ausgabe statt in eine Übersetzung abgerutscht sein,
...
## Wenn Datei und Übersetzungsspeicher sich widersprechen

Der eigene Übersetzungsspeicher des Projekts gewinnt. Hat der Übersetzungsspeicher für dieselbe Quelle und Locale schon eine andere Übersetzung, wird die importierte abgelehnt und als behalten gezählt. Mit `--overwrite` drehst du das um und lässt die Datei gewinnen.
...
So eine Wiederverwendung unterliegt weiterhin dem Integritäts-Gate gegen deinen echten Eintrag und wird in der Lauf-Zusammenfassung als Review-Flag `FUZZY_CACHE_REUSE` gemeldet, ist also sichtbar und nicht still. Soll ein importierter Übersetzungsspeicher auf diesem Weg gar nicht erreichbar sein, lass `fuzzyCache` aus deiner Konfiguration weg: ohne diese Option wird nur ein exakter Content-Hash-Treffer je wiederverwendet.
...
| `2` | konnte nicht laufen: ein Konfigurationsfehler, eine fehlende oder unlesbare Datei, eine Datei, die kein gültiges TMX ist, oder ein Verwendungsfehler |

Eine Datei, die zu groß ist, fehlerhaft ist, kein TMX-Dokument ist oder eine XML-Entity deklariert, wird mit einem strukturierten Fehler abgelehnt, der benennt, was schiefging. Hat das Problem einen Ort in der Datei, nennt die Meldung auch Zeile und Spalte und, innerhalb einer Übersetzungseinheit, welche Einheit es war (ab 1 gezählt), sodass `line 9, column 29, unit 2` dich zum zweiten `tu` führt. Der Code des Fehlers ist `SOURCE_INVALID`; mit `--json` steht dieselbe Meldung im Fehler-Envelope, und wer das SDK aufruft, liest denselben Ort mit `tmxErrorLocation(error)` als strukturierte Daten aus, wobei die zugrunde liegende `cause` ein Exchange-Fehler mit dem Code `TMX_INVALID` ist. Entity-Deklarationen und interne DTD-Subsets werden rundweg abgelehnt, sodass weder eine externe Entity-Referenz noch eine unbegrenzte Expansion je erreicht wird, und die schlichte externe Doctype-Zeile, die echte Werkzeuge schreiben, wird verworfen statt geladen.

Übersprungene oder abgelehnte Einheiten lassen den Lauf nicht scheitern. Sie werden gemeldet, denn ein Übersetzungsspeicher aus der Praxis trägt fast immer von beidem etwas.

## Siehe auch

- [Der Cache](/docs/the-cache) erklärt, was der Übersetzungsspeicher ist und woraus sein Key gebildet wird.
- [Menschliche Übersetzung](/docs/manual-translation) behandelt die Übergabe als xlsx, CSV und TSV, und das ist etwas anderes: ein Ausschnitt des offenen Deltas pro Locale für eine Übersetzerin, nicht der ganze Übersetzungsspeicher.
