...
2. Andernfalls kann das Tag nur eine konfigurierte Locale erreichen, die es Subtag für Subtag erweitert oder von der es erweitert wird. Eine Datei in `en-US` landet also in einem Projekt, das `en` konfiguriert hat, und eine Datei in `pt` landet in einem Projekt, das nur `pt-BR` konfiguriert hat, solange genau eine konfigurierte Locale in Frage kommt.
3. Zwei Tags, deren Schrift- oder Regions-Subtags sich unterscheiden, passen nie zusammen. `zh-CN` wird nie als `zh-TW` gespeichert, `sr-Latn` nie als `sr-Cyrl` und `de-AT` nie als `de-CH`: So ein Tag wird als zu keiner konfigurierten Locale passend gemeldet, nicht geraten.
4. Könnten zwei konfigurierte Locales das Tag gleichermaßen beanspruchen, wird nichts gewählt. `pt` in einem Projekt mit `pt-BR` und `pt-AO` wird als mehrdeutig gemeldet, nicht derjenigen zugeschlagen, die zuerst kam.
