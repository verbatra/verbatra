...
t<string>("nav.next");                      // argument de type explicite
```
...
| Signalé comme | Quand |
| --- | --- |
| `dynamic` | l'argument de la clé n'est pas une seule chaîne statique complète : une variable, un accès de propriété, un littéral de gabarit contenant une expression, une concaténation ou une clé qualifiée par un espace de noms. Elle n'est jamais écrite avec une clé devinée ou tronquée. |
