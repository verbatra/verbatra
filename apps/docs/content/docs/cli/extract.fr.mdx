...
`extract` est pilotée par le [bloc `extract`](/docs/config-file#extract) de ta config. Une exécution sans lui échoue avec `EXTRACT_NOT_CONFIGURED` et le code de sortie `2`.
...
Le framework `i18next` lit les formes d'appel `t(...)`, `$t(...)` et `<object>.t(...)` dans les fichiers `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.mts` et `.cts`. La clé est le premier argument. Une valeur par défaut est soit le deuxième argument, soit un champ `defaultValue` sur l'objet d'options :
...
- Une clé du catalogue qu'aucun appel ne mentionne reste intacte. `extract` ajoute et signale ; elle ne supprime jamais.
...
| Signalé comme | Quand |
| --- | --- |
| `dynamic` | l'argument de la clé n'est pas une chaîne statique : une variable, un accès de propriété ou un littéral de gabarit contenant une expression. Elle n'est jamais écrite avec une clé devinée. |
...
| `diagnostics` | un fichier ou un répertoire n'a pas pu être lu : il a disparu, il dépasse la limite de taille, ou l'analyse a échoué dessus. |
