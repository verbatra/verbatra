...
The two entry writers, `translation.editEntry` and `translation.retranslateEntry`, are additionally guarded per locale and key: while one call for a given entry is still running, a second call naming the same `locale` and `key` is refused with `ALREADY_IN_PROGRESS` rather than queued or run alongside it, so two agents (or one agent retrying) cannot race on the same entry. The guard is per entry, not per tool, so calls for different keys or different locales still run concurrently.
