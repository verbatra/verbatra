...
2. Otherwise, the tag can only reach a configured locale that it extends or that extends it, subtag by subtag. A file written in `en-US` therefore lands in a project configured for `en`, and a file written in `pt` lands in a project configured only for `pt-BR`, as long as exactly one configured locale qualifies.
3. Two tags whose script or region subtags differ never match. `zh-CN` is never stored as `zh-TW`, `sr-Latn` never as `sr-Cyrl`, and `de-AT` never as `de-CH`: such a tag is reported as matching no configured locale, not guessed at.
4. If two configured locales could equally claim the tag, nothing is chosen. `pt` in a project configured for `pt-BR` and `pt-AO` is reported as ambiguous, not assigned to whichever came first.
