...
One consequence is worth stating plainly, because no check at import time can police it. An accepted unit's source text is written into the memory's source index, which is exactly what [fuzzy reuse](/docs/the-cache) scores a changed string against. An imported source that **differs** from a string in your project can therefore be served for that string, which is what resemblance means.
