...
2. Sinon, l'étiquette ne peut atteindre qu'une locale configurée qu'elle prolonge ou qui la prolonge, sous-étiquette par sous-étiquette. Un fichier écrit en `en-US` atterrit ainsi dans un projet configuré en `en`, et un fichier écrit en `pt` atterrit dans un projet configuré uniquement en `pt-BR`, tant qu'une seule locale configurée convient.
3. Deux étiquettes dont les sous-étiquettes d'écriture ou de région diffèrent ne correspondent jamais. `zh-CN` n'est jamais stocké comme `zh-TW`, `sr-Latn` jamais comme `sr-Cyrl` et `de-AT` jamais comme `de-CH` : une telle étiquette est signalée comme ne correspondant à aucune locale configurée, pas devinée.
4. Si deux locales configurées pouvaient revendiquer l'étiquette à égalité, aucune n'est choisie. `pt` dans un projet avec `pt-BR` et `pt-AO` est signalé comme ambigu, et non attribué à celle qui venait en premier.
...
Une conséquence mérite d'être dite clairement, car aucune vérification au moment de l'import ne peut la surveiller. Le texte source d'une unité acceptée est écrit dans l'index des sources de la mémoire, c'est-à-dire exactement ce contre quoi la [réutilisation floue](/docs/the-cache) évalue une chaîne modifiée. Une source importée qui **diffère** d'une chaîne de ton projet peut donc être servie pour cette chaîne : c'est ce que ressembler veut dire.
...
Un fichier surdimensionné, mal formé, qui n'est pas un document TMX ou qui déclare une entité XML est refusé par une erreur structurée nommant ce qui a échoué. Quand le problème a un emplacement dans le fichier, le message indique aussi la ligne et la colonne et, à l'intérieur d'une unité de traduction, de quelle unité il s'agit (en comptant à partir de 1), si bien que `line 9, column 29, unit 2` te mène au deuxième `tu`. Avec `--json`, le même message figure dans l'enveloppe d'erreur, et qui appelle le SDK trouve le même emplacement sous forme de données structurées dans le `cause` de l'erreur. Les déclarations d'entité et les sous-ensembles DTD internes sont refusés d'emblée, si bien que ni une référence d'entité externe ni une expansion sans borne ne sont jamais atteintes, et la simple ligne doctype externe qu'écrivent les outils réels est écartée plutôt que chargée.
