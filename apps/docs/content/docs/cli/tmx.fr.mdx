...
5. Une étiquette n'est signalée comme ambiguë que lorsque les locales configurées qu'elle pourrait atteindre ne se trouvent pas sur une même chaîne de préfixes. `pt` dans un projet avec `pt-BR` et `pt-AO` est signalé comme ambigu, et non attribué à celle qui venait en premier.
