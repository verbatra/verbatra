...
All eight shipped adapters (i18next, vue-i18n, next-intl, ngx-translate, XLIFF, YAML, Flutter ARB,
Java/Spring properties) are registered in `createDefaultRegistry`
...
- DeepL is the only `machine-translation` provider and implements `translateBatch` directly
  (`packages/ai-providers/src/deepl/deepl-provider.ts`), since it takes strings and returns
  strings with no prompt.
...
"openai-compatible"`), so a provider present in the config union but missing from the factory
table fails to compile. `buildProvider(config)` reads this table; `selectProvider`
