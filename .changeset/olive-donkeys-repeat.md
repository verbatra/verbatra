...
Both write by patching the destination document in place, so comments, spacing, the `.resx` schema preamble, and `.resx` entries holding serialized objects or designer metadata survive untouched. Both are private-package sources that tsup bundles into the published SDK, so the bump lands here.
