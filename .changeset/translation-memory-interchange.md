...
place in the file, its line, column and, inside a translation unit, the unit's 1-based ordinal. The
error's `cause` is the interchange reader's error carrying the same place as a structured `location`
(typed as `TmxErrorLocation`), and `verbatra tmx import` prints that message on its error line and
in the `--json` error envelope, so a mismatched closing tag in the second unit and a truncated file
are reported as two distinct, located refusals.
...
only when one of the two is a strict subtag prefix of the other, and only when exactly one
configured locale qualifies: a file written in `en-US` lands in a project configured for `en`, and a
file written in `pt` lands in a project configured only for `pt-BR`. Two tags whose script or region
subtags differ never match, so `zh-CN` is never stored as `zh-TW`, `sr-Latn` never as `sr-Cyrl`, and
`de-AT` never as `de-CH`; such a tag is reported as unmatched rather than guessed at, and a tag
several configured locales could claim is reported as ambiguous. Resolving against the whole
configured set in one pass is what keeps a regional target such as `pt-BR` from being taken for the
bare `pt` source it extends. A unit carrying two segments that both resolve to the source locale is
refused rather than attributed to whichever came last, and a config whose source and one target are
the same tag once normalized is refused outright.
